...
public class PageInfo {
	...
	private int listCount;			// 현재 총 게시글 갯수 (일반게시판,유효한게시판) 이거에 따라 마지막 페이지수가 달라짐
	private int currentPage;		// 현재 페이지(즉, 사용자가 요청한 페이지)
	private int pageLimit;			// 페이지 하단에 보여질 페이징바의 페이지 최대 갯수
	private int boardLimit;			// 한 페이지내에 보여질 게시글 최대갯수(몇개 단위씩)
	...
	public PageInfo(int pageLimit, int maxPage, int startPage, int endPage) {
		super();
		this.pageLimit = pageLimit;
		this.maxPage = maxPage;
		this.startPage = startPage;
